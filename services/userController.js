...
usercontroller.prototype.findById = function (userId, callback) {
    var findUserQuery = 'SELECT u.first_name,u.last_name,u.email,u.profile_img,r.role_name as `role` FROM users u,roles r WHERE u.role_id = r.id AND u.id=?';
    var params = [userId];
...
usercontroller.prototype.authenticate = function (email, password, callback) {
    var authQuery = 'SELECT u.id,u.first_name,u.last_name,u.email,r.role_name as `role` FROM users u, auth a, roles r WHERE u.id=a.user_id AND u.role_id = r.id AND u.email=? AND a.md5passwd=md5(?)';
